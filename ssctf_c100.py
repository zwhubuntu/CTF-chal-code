...
ch = 'OO|OO||OO|||||OO|OO||O||O|O||O|||O|OOOOOOO|O|O|O|||||OO|||O|||OO||O|OOOOOO|O|OO|OO||||OO|||OOOO|||||O||||O|OO|O|O|O||OO|O||O|OO|O||O|||O||O|OO|OOOOOO||OOO|O|O|O|||O|OO|O|O||O||O||OOOOO|||OO|O|'
...
def decrypt(kkkk, flag):
    ...
    for d in range(27):
        for i in charset:
            xx = encrypt1(kkkk, f + i)
            if xx[:len(f+i)*8] == flag[:len(f+i)*8]:
                f=f+i
    print f
                
            
    
kkk = chr(k0[0]) + chr(k1[0])+ chr(k2[0])+ chr(k3[0])+ chr(k4[0])+ chr(k5[0])+ chr(k6[0])+ chr(k7[0])
for i0 in k0:
    for i1 in k1:
        for i2 in k2:
            for i3 in k3:
                for i4 in k4:
                    for i5 in k5:
                        for i6 in k6:
                            for i7 in k7:
                                k99 = chr(i0) + chr(i1)+ chr(i2)+ chr(i3)+ chr(i4)+ chr(i5)+ chr(i6)+ chr(i7)
                                
                                #print len(encrypt(k99)), len(ch)
                                if encrypt(k99) == ch[:len(encrypt(k99))]:
                                    print k99.encode('hex'), k99
                                    decrypt(k99, flag)
