...
#key = bytearray("abcd1234")
print "cracking....."
for t in itertools.product(chars, repeat=8):
    w = ''.join(t)
    #print "testing:"
    #print w
    key = bytearray(w)
    ...
    for i in t2:
        out += encode(i)
    if out=="OO|OO||OO|||||OO|OO||O||O|O||O|||O|OOOOOOO|O|O|O|||||OO|||O|||OO||O|OOOOOO|O|OO|OO||||OO|||OOOO|||||O||||O|OO|O|O|O||OO|O||O|OO|O||O|||O||O|OO|OOOOOO||OOO|O|O|O|||O|OO|O|O||O||O||OOOOO|||OO|O|":
        print "result:"
        print w
        break;
print "accomplished!"
